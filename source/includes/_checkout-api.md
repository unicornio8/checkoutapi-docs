...
As mentioned above, payment is done in an individual component that does not belong to team Atlas. One of the reasons for this is PCI compliance. This means that customers will be directed to a different service where they will select the payment method, and input whatever information is necessary to purchase with that method. Fortunately, Zalando's payment component allows customization, which enables a seamless transition. From our customers' perspective it can all have the same look and feel.
